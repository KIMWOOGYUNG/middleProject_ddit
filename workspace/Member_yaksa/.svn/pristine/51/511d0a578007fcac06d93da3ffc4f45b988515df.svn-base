...
import yaksa.member.service.join.CalendarService;
import yaksa.member.service.join.ICalendarService;
...
public class calendarController {
    ...
    void addclcik(ActionEvent event) throws IOException {
    	...
    	//새창 
    	Parent add = FXMLLoader.load(getClass().getResource("/yaksa/member/join/inputCalendar.fxml"));
                	...
                	clickDate = ((Label)(((VBox)(ee.getSource())).getChildren().get(1))).getText();
					 ...
					 if(e.getClickCount()==2) {
						 ...
						 try {
							 ...
							 FXMLLoader loader = new FXMLLoader(calendarController.class.getResource("/yaksa/member/fxml/join/detailCalendar.fxml"));
...
//    	}
    	...
    	changeCalendar(nYear, nMonth);
         ...
         try {
            ...
            service = (ICalendarService) reg.lookup("calService");
            //service = CalendarService.getInstance();
    ...
    public void changeCalendar(int nYear, int nMonth) {
    	...
    	for(int i=0; i<hboxList.length; i++) {
    		...
    		hboxList[i].setStyle("-fx-border-color:  #73bbff;");
    	}
    	//현재 날짜 얻어오기
