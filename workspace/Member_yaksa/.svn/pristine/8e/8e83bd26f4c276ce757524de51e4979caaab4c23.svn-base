...
//	private WebView MapView;
	...
	void MaskSaleMap(ActionEvent event) {
		...
		try {
			...
			FXMLLoader insert = new FXMLLoader(ContYKSearch.class.getResource("/yaksa/member/fxml/map/MaskSaleYK.fxml"));
						...
						btn2.setOnAction(e -> {
							try {
								...
								FXMLLoader detail = new FXMLLoader(
										ContYKSearch.class.getResource("/yaksa/member/fxml/map/YKSearchDetail.fxml"));
...
//		// ============= webview에서 값을 받음 
		...
		webEngine.load("http://localhost:8081/Map/NewFile1.html"); // 정민이가 수정한 페이지
