...
public class MemberLoginController {
    ...
    void Join(ActionEvent event) {		// 회원가입 창 불러오기
    	try {
    		...
    		Parent childRoot = FXMLLoader.load(MemberLoginController.class.getResource("/yaksa/member/fxml/join/Agree.fxml"));
...
//			
			...
			if(loginMember!=null) {
				...
				//세션에 저장
				if(SessionVO.loginMember != null) {
					login_state = true;
				}
				
				SessionVO.loginMember = loginMember;
						
				AlertUtil.infoMsg("로그인 성공!", id + "님 반갑습니다:)");
				System.out.println("***"+SessionVO.loginMember);
				MainController.memVO = loginMember;
				Stage dialog = new Stage();
				FXMLLoader insert = new FXMLLoader(MemberLoginController.class.getResource("/yaksa/member/fxml/main/Main.fxml"));
...
//				main.login(member);
    ...
    void LookId(ActionEvent event) {	// 아이디 찾기 구현
    	try {
	    	...
	    	Stage dialog = new Stage();
			Parent childRoot = FXMLLoader.load(MemberLoginController.class.getResource("/yaksa/member/fxml/join/SearchMemId.fxml"));
			dialog.initStyle(StageStyle.TRANSPARENT); //
    		Scene childScene = new Scene(childRoot);
    		childScene.setFill(Color.TRANSPARENT);	//
    ...
    void LookPwd(ActionEvent event) {	// 비밀번호 찾기 구현
    	try {
	    	...
	    	Stage dialog = new Stage();
			Parent childRoot = FXMLLoader.load(MemberLoginController.class.getResource("/yaksa/member/fxml/join/SearchMemPw.fxml"));
			dialog.initStyle(StageStyle.TRANSPARENT); //
    		Scene childScene = new Scene(childRoot);
    		childScene.setFill(Color.TRANSPARENT);	//
