...
public class PresOfChemiController {
    ...
    void setID(InboxVO vo) {
    	...
    	paycode.setText(vo.getPhar_file());
    	payDate.setText(dateStr);
    	
    	int price2 = 0;
    	
    	for(int i = 0; i < PreviewController.saleList.size(); i++) {
    		price2 += PreviewController.saleList.get(i).getSale_sumprice();
    	}
    	price.setText(Integer.toString(price2));
 	
    	System.out.println(memVo.getMem_tel());
    	
    	tel_mem = memVo.getMem_tel();
